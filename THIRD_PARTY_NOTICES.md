...
| `to-spec-onward` | [to-spec](https://github.com/mattpocock/skills/blob/3cca18b368ae95cdbdebbff572ccafa662551015/skills/engineering/to-spec/SKILL.md) | 保留综合讨论、完整规范模板和测试决定等要求；追踪器前提改为项目约定，发布议题改为本地文档；保留仅手动调用语义 |
| `plan-onward` | [to-tickets](https://github.com/mattpocock/skills/blob/3cca18b368ae95cdbdebbff572ccafa662551015/skills/engineering/to-tickets/SKILL.md) | 保留纵向切片、依赖、任务粒度、用户确认、大范围重构例外及本地模板；输出限定为本地任务文件，增加计划更新和规范引用，使用项目约定；保留仅手动调用语义 |
| `implement-onward` | [implement](https://github.com/mattpocock/skills/blob/3cca18b368ae95cdbdebbff572ccafa662551015/skills/engineering/implement/SKILL.md) | 保留 TDD、检查频率、审查和提交；限定指定规范或当前可执行任务，保护已有改动，通过 tidy 同步文档，再调用共用审查，收尾报告后停止；保留仅手动调用语义 |
...
| `domain-modeling-onward` | [domain-modeling](https://github.com/mattpocock/skills/blob/3cca18b368ae95cdbdebbff572ccafa662551015/skills/engineering/domain-modeling/SKILL.md) | 方法正文及领域上下文、架构决策格式参考保留；允许自动与手动调用 |
...
| `diagnosing-bugs-onward` | [diagnosing-bugs](https://github.com/mattpocock/skills/blob/3cca18b368ae95cdbdebbff572ccafa662551015/skills/engineering/diagnosing-bugs/SKILL.md) | 完整六阶段方法、脱敏、复现、假设、回归要求及人工配合脚本保留；增加读取项目约定的前提；允许自动与手动调用 |

`agents/openai.yaml` 是本包使用的 Codex 界面与调用配置。原技能的仅手动调用要求在适用时转换为 `allow_implicit_invocation: false`。`grilling-onward`、`to-spec-onward` 和 `plan-onward` 只保留调用策略，未复制来源适配中的界面展示字段；`implement-onward` 同时更新界面名称与短描述，其余保留的界面名称加入 Onward。其他新增或保留的自动调用选择见 [README](README.md)。
