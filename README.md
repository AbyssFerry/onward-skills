...
项目尚无文档结构时，`init-onward` 按默认结构初始化。已有结构时，先只读查看，参照技能正文的简短示例，用“处理方式、当前文件、推荐位置、用途与原因”四列表格展示推荐迁移方案，并展示拟写入的 Onward 区块和关键工程约定草稿。你可以全部或部分保留旧位置，也可以自定义；最终方案确认后写入 AGENTS.md，再执行初始化。迁移由 init 在初始化中完成，保留内容并同步引用与约定。日常文档整理仍由 `tidy-onward` 处理。
