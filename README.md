...
项目尚无文档结构时，`init-onward` 按默认结构初始化。已有结构时，先只读查看，用带用途注释的目录树或路径对照展示推荐迁移方案，列明具体文件的当前位置、建议位置与用途，并展示拟写入的 Onward 区块和关键工程约定草稿。你可以全部或部分保留旧位置，也可以自定义；最终方案确认后写入 AGENTS.md，再执行初始化。迁移由 init 在初始化中完成，保留内容并同步引用与约定。日常文档整理仍由 `tidy-onward` 处理。
